...
def build_pcss_eval_split(
    *,
    ...
    split_sets = {
        "guide_ids": set(guide_ids),
        "final_ids": set(final_ids),
        "pool_ids": set(pool_ids),
    }
    for left_name, right_name in (("guide_ids", "final_ids"), ("guide_ids", "pool_ids"), ("final_ids", "pool_ids")):
        overlap = sorted(split_sets[left_name] & split_sets[right_name])
        if overlap:
            raise ValueError(
                f"{left_name} and {right_name} must be disjoint; "
                f"first={overlap[0]!r}, overlap_count={len(overlap)}"
            )
