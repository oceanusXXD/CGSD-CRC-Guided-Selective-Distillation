...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model_path", default="models/base-model")
...
def _server_model_names(args: argparse.Namespace) -> tuple[str, str]:
    base_name = str(args.served_model_name or Path(str(args.model_path)).name or "base-model")
...
def _start_vllm_server(args: argparse.Namespace, *, checkpoint_dir: Path | None) -> subprocess.Popen[str]:
    ...
    model_path = input_artifact_path(args.model_path, PROJECT_ROOT / "models" / "base-model")
    ...
    log_path = (
        output_artifact_path(args.server_log_path, PROJECT_ROOT / "runs" / "vllm_openai.log")
        if args.server_log_path
        else PROJECT_ROOT / "runs" / "vllm_openai.log"
    )
