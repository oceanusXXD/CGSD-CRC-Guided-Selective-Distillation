...
LORA_TARGETS: dict[str, list[str]] = {
    "lora_attention_mlp": LORA_TARGET_GROUPS["attention_mlp"],
}
    ...
    if class_token_weights:
        for token_id, weight in class_token_weights.items():
            token_weights = torch.where(
                active_labels.eq(int(token_id)),
                torch.as_tensor(float(weight), dtype=per_token_loss.dtype, device=per_token_loss.device),
                token_weights,
            )
...
class GenerativeBinaryModel(nn.Module):
    ...
    def __init__(
        ...
        lora_target_modules: str = "attention_mlp",
        lora_layer_scope: str = "last1",
        ...
        self.lora_layers_to_transform = resolve_lora_layers_to_transform(
            loaded_model.config,
    ...
    def _build_lora_backbone(
        ...
        config_kwargs = {
            ...
            "target_modules": LORA_TARGET_GROUPS[self.lora_target_modules],
    ...
    def checkpoint_config(self) -> dict[str, Any]:
        return {
            ...
            "lora_layer_scope": self.lora_layer_scope,
            "lora_layers_to_transform": self.lora_layers_to_transform,
            "lora_layers_pattern": LAST_LAYER_LORA_PATTERN if self.lora_layers_to_transform is not None else None,
