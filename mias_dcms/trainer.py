...
def train_one_epoch(model: torch.nn.Module, dataloader: DataLoader, optimizer: torch.optim.Optimizer, scheduler: Any, device: torch.device, gradient_accumulation_steps: int=1, max_grad_norm: float=1.0, class_token_weights: dict[int, float] | None=None) -> float:
    ...
    for step, batch in enumerate(progress, start=1):
        ...
        outputs = model(**batch, class_token_weights=class_token_weights, sample_weights=sample_weights)
...
@torch.no_grad()
def evaluate_model(model: torch.nn.Module, dataloader: DataLoader, device: torch.device, threshold: float=0.0, predictions_path: str | Path | None=None, tokenizer: Any | None=None, negative_token_text: str=BINARY_NEGATIVE_TEXT, positive_token_text: str=BINARY_POSITIVE_TEXT) -> dict[str, Any]:
    ...
    for batch in tqdm(dataloader, desc='eval', leave=False):
        ...
        prompt_outputs = model(input_ids=batch['prompt_input_ids'], attention_mask=batch['prompt_attention_mask'])
        next_token_logits = last_non_padding_logits(prompt_outputs.logits, batch['prompt_attention_mask'])
        ...
        for sample_id, label, score, probability, zero_logit, one_logit in zip(sample_ids, batch_labels, batch_scores, probs, batch_zero_logits, batch_one_logits):
            prediction = 1 if float(score) > threshold else 0
            rows.append({'id': sample_id, 'label': int(label), 'score': float(score), 'zero_logit': float(zero_logit), 'one_logit': float(one_logit), 'probability': float(probability), 'prediction': prediction, 'generated_text': str(int(prediction)), 'score_source': score_source})
...
@torch.no_grad()
def predict_model(model: torch.nn.Module, dataloader: DataLoader, device: torch.device, tokenizer: Any, threshold: float=0.0, negative_token_text: str=BINARY_NEGATIVE_TEXT, positive_token_text: str=BINARY_POSITIVE_TEXT, predictions_path: str | Path | None=None, row_callback: Callable[[dict[str, Any]], None] | None=None) -> list[dict[str, Any]]:
    model.eval()
    negative_token_id = get_single_token_id(tokenizer, negative_token_text)
    positive_token_id = get_single_token_id(tokenizer, positive_token_text)
    rows: list[dict[str, Any]] = []
    ...
    try:
        for batch in tqdm(dataloader, desc='predict', leave=False):
            ...
            for sample_id, label, score, probability, zero_logit, one_logit in zip(sample_ids, batch_labels, batch_scores, probs, batch_zero_logits, batch_one_logits):
                prediction = 1 if float(score) > threshold else 0
                row = {'id': str(sample_id), 'label': int(label), 'score': float(score), 'zero_logit': float(zero_logit), 'one_logit': float(one_logit), 'probability': float(probability), 'prediction': int(prediction), 'generated_text': str(int(prediction)), 'score_source': score_source}
                rows.append(row)
...
def fit(model: torch.nn.Module, train_loader: DataLoader, eval_loader: DataLoader | None, tokenizer: Any, output_dir: str | Path, device: torch.device, epochs: int, lr: float, weight_decay: float, gradient_accumulation_steps: int, max_grad_norm: float, warmup_ratio: float, threshold: float, run_config: dict[str, Any], early_stopping_patience: int | None=None, early_stopping_min_delta: float=0.0, class_token_weights: dict[int, float] | None=None, scheduler_type: str='linear') -> dict[str, Any]:
    model.to(device)
    ...
    for epoch in range(1, epochs + 1):
        train_loss = train_one_epoch(model=model, dataloader=train_loader, optimizer=optimizer, scheduler=scheduler, device=device, gradient_accumulation_steps=gradient_accumulation_steps, max_grad_norm=max_grad_norm, class_token_weights=class_token_weights)
        epoch_record: dict[str, Any] = {'epoch': epoch, 'train_loss': train_loss}
        if eval_loader is not None:
            eval_metrics = evaluate_model(model=model, dataloader=eval_loader, device=device, threshold=threshold, tokenizer=tokenizer)
        ...
        write_json({'history': history, 'requested_epochs': epochs, 'completed_epochs': len(history), 'early_stopped': False, 'run_complete': False}, output_path / 'training_state.json')
        ...
        model.save_checkpoint(output_path / f'epoch_{epoch}', tokenizer=tokenizer, extra_config=epoch_config)
        if eval_loader is not None:
            ...
            if metric > best_metric + early_stopping_min_delta:
                ...
                model.save_checkpoint(output_dir, tokenizer=tokenizer, extra_config=checkpoint_config)
            else:
                ...
                if early_stopping_patience is not None and early_stopping_patience >= 0 and (epochs_without_improvement >= early_stopping_patience):
                    print(json.dumps({'early_stopping': {'best_epoch': best_epoch, 'best_metric': best_metric, 'epoch': epoch, 'patience': early_stopping_patience}}, ensure_ascii=False, sort_keys=True))
                    early_stopped = True
                    break
