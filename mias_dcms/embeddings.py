...
def _embedding_ids_from_sidecar(path: Path, row_count: int) -> list[str]:
    candidates = [path.with_suffix('.ids.jsonl'), path.with_suffix('.ids.json'), path.parent / 'embedding_rows.jsonl']
    for candidate in candidates:
        if not candidate.exists():
            continue
        if candidate.suffix == '.jsonl':
            ids = [str(row.get('id', row.get('sample_id', ''))) for row in read_jsonl(candidate)]
        else:
        ...
        if len(ids) == int(row_count):
            ...
            return ids
    raise ValueError(f'{path} is a .npy embedding matrix with {row_count} rows, but no matching id sidecar was found. Expected *.ids.jsonl, *.ids.json, or embedding_rows.jsonl.')
...
def load_embeddings(path: str | Path) -> dict[str, np.ndarray]:
    ...
    if source.suffix == '.npy':
        ...
        return {sample_id: np.asarray(matrix[index], dtype=np.float32) for index, sample_id in enumerate(ids)}
...
def assert_embedding_coverage(embeddings_by_id: dict[str, np.ndarray], rows: list[dict[str, Any]], *, expected_dim: int) -> None:
    ...
    if bad_shape:
        raise ValueError(f'embeddings_path has vectors with invalid shape; expected ({expected_dim},), examples: {bad_shape[:5]}')
