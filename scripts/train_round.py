...
from scripts.cli_common import add_runtime_overrides, add_stage_cache_args, estimate_query_document_prompt_tokens, input_artifact_path, load_selected_train_rows, load_stage_examples, output_dir_from_arg, output_artifact_path, print_existing_stage_result, read_jsonl, runtime_args_from_cli, selected_train_rows_path, split_ids_path, stage_cache_decision, summarize_teacher_label_usage, train_label_snapshot, write_stage_usage
from mias_dcms.binary_protocol import BINARY_NEGATIVE_TEXT, BINARY_POSITIVE_TEXT
from mias_dcms.data import GenerationPairCollator, GenerationQueryDocumentDataset, PairExample, examples_from_rows, filter_examples_by_ids
from mias_dcms.model import QwenGenerativeModel
from mias_dcms.trainer import fit
from mias_dcms.utils import configure_torch_performance, disable_tokenizer_thinking, ensure_tokenizer_padding, get_device, parse_torch_dtype, read_json, set_seed, write_json, write_jsonl
...
def build_train_dataloader(examples: list[PairExample], tokenizer: Any, *, max_length: int, batch_size: int, shuffle: bool, num_workers: int, prefetch_factor: int, pin_memory: bool, pad_to_multiple_of: int, cache_tokenization: bool) -> Any:
    from torch.utils.data import DataLoader
    dataset = GenerationQueryDocumentDataset(examples, tokenizer=tokenizer, max_length=max_length, cache_tokenization=cache_tokenization, input_format='chat_binary')
    kwargs: dict[str, Any] = {'dataset': dataset, 'batch_size': batch_size, 'shuffle': shuffle, 'collate_fn': GenerationPairCollator(tokenizer, pad_to_multiple_of=pad_to_multiple_of if pad_to_multiple_of > 0 else None), 'num_workers': num_workers, 'pin_memory': pin_memory}
    if num_workers > 0:
...
def train_round_model(*, train_examples: list[PairExample], eval_examples: list[PairExample], tokenizer: Any, model_path: Path, init_adapter_path: Path | None, output_dir: Path, device: Any, args: Any, round_index: int, run_config: dict[str, Any]) -> QwenGenerativeModel:
    ...
    trained_model = QwenGenerativeModel.load_from_checkpoint(output_dir, torch_dtype=parse_torch_dtype(args.torch_dtype), model_path=model_path)
...
def main() -> None:
    ...
    training_rows_used_path = output_artifact_path(args.training_rows_used_path, round_dir / 'training_rows_used.jsonl')
    train_label_snapshot_path = output_artifact_path(args.train_label_snapshot_path, round_dir / 'train_label_snapshot.json')
    training_summary_path = output_artifact_path(args.training_summary_path, round_dir / 'training_round_summary.json')
    ...
    if args.show_result:
        ...
        return
    cache_decision = stage_cache_decision(stage_name='train_round', required_outputs=[checkpoint_dir / 'model_config.json', training_rows_used_path, train_label_snapshot_path, training_summary_path, usage_path], cache_policy=args.cache_policy)
    ...
    training_rows_snapshot = [{**dict(row), 'trained_for_round': int(args.round_index), 'training_artifact_role': 'cumulative_lora_train_row'} for row in selected_rows]
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=runtime_args.trust_remote_code, local_files_only=True)
    ...
    print(json.dumps({'round_index': args.round_index, 'checkpoint_dir': str(checkpoint_dir), 'train_size': len(selected_rows), 'selection_round_counts': selection_round_counts}, ensure_ascii=False, sort_keys=True))
