...
from scripts.cli_common import add_stage_cache_args, embedding_usage_payload, output_dir_from_arg, output_artifact_path, print_existing_stage_result, stage_cache_decision, write_stage_usage
from mias_dcms.binary_protocol import normalize_binary_label
from mias_dcms.data import examples_to_rows, load_examples
from mias_dcms.embeddings import assert_embedding_coverage, load_embeddings
from mias_dcms.utils import resolve_input_path, set_seed, write_json
...
def _label_distribution(rows_by_id: dict[str, dict[str, object]], ids: list[str]) -> dict[str, object]:
    ...
    size = len(ids)
    return {'label0_count': int(size - label1_count), 'label1_count': int(label1_count), 'size': int(size), 'label1_rate': float(label1_count / size) if size else 0.0}
...
def _take_stratified_ids(*, remaining_by_label: dict[int, list[str]], requested_size: int) -> list[str]:
    if requested_size <= 0:
        return []
    label1_count = _allocate_label1_count(requested_size=requested_size, remaining_label0=len(remaining_by_label[0]), remaining_label1=len(remaining_by_label[1]))
...
def _split_ids_stratified(rows: list[dict[str, object]], *, n_guide: int, n_final: int, rng: random.Random) -> tuple[list[str], list[str], list[str]]:
...
def _split_ids_random(rows: list[dict[str, object]], *, n_guide: int, n_final: int, rng: random.Random) -> tuple[list[str], list[str], list[str]]:
...
def main() -> None:
    ...
    if args.show_result:
        ...
        return
    cache_decision = stage_cache_decision(stage_name='prepare', required_outputs=[split_ids_path, usage_path], cache_policy=args.cache_policy)
    if cache_decision.cache_hit:
        print_existing_stage_result(stage_name='prepare', summary_path=split_ids_path)
        return
    output_dir.mkdir(parents=True, exist_ok=True)
    data_path = resolve_input_path(args.data_path, PROJECT_ROOT)
    examples = load_examples(data_path, query_field=args.query_field, document_field=args.document_field, label_field=args.label_field)
    ...
    label_distribution = {'all': _label_distribution(rows_by_id, [_row_id(row) for row in rows]), 'guide': _label_distribution(rows_by_id, guide_ids), 'final': _label_distribution(rows_by_id, final_ids), 'pool': _label_distribution(rows_by_id, pool_ids)}
    ...
    if args.embeddings_path:
        ...
        embedding_usage = embedding_usage_payload(embedding_source=embeddings_path, row_count=len(rows), embedding_dim=int(args.embedding_dim), purpose='prepare_validate_full_embedding_coverage')
    split_payload = {'guide_ids': guide_ids, 'final_ids': final_ids, 'pool_ids': pool_ids, 'n_guide': n_guide, 'n_final': n_final, 'seed': args.seed, 'split_algorithm': split_algorithm, 'split_strategy': str(args.split_strategy), 'label_distribution': label_distribution}
    write_json(split_payload, split_ids_path)
    usage_payload = {'stage_name': 'prepare', 'cache': cache_decision.to_dict(), 'data_path': str(data_path), 'split_ids_path': str(split_ids_path), 'data_rows': len(rows), 'guide_size': len(guide_ids), 'final_size': len(final_ids), 'pool_size': len(pool_ids), 'split_strategy': str(args.split_strategy), 'label_distribution': label_distribution, 'student_model_calls': 0, 'teacher_calls': 0, 'groundtruth_substitute_calls': 0}
