...
def build_pcss_eval_split(*, data_path: str | Path, split_ids_path: str | Path, train_rows_path: str | Path, output_path: str | Path, summary_path: str | Path) -> dict[str, Any]:
    ...
    if train_locked_overlap:
        raise ValueError(f'selected training rows overlaps locked guide/final ids; first={train_locked_overlap[0]!r}, overlap_count={len(train_locked_overlap)}')
    pool_id_set = set(pool_ids)
    train_not_pool = sorted(train_id_set - pool_id_set)
    if train_not_pool:
        raise ValueError(f'selected training rows must come from the PCSS pool; first={train_not_pool[0]!r}, count={len(train_not_pool)}')
    pool_test_ids = [sample_id for sample_id in pool_ids if sample_id not in train_id_set]
    eval_split = {'guide_ids': [], 'final_ids': final_ids, 'pool_ids': pool_test_ids}
    summary = {'stage_name': 'make_pcss_eval_split', 'data_path': str(data_source), 'split_ids_path': str(split_source), 'train_rows_path': str(train_source), 'output_path': str(output), 'summary_path': str(summary_output), 'data_count': len(data_ids), 'guide_count': len(guide_ids), 'cert_count': len(final_ids), 'pool_candidate_count': len(pool_ids), 'selected_train_count': len(train_ids), 'pool_test_count': len(pool_test_ids), 'pool_test_label_counts': _label_counts(pool_test_ids, rows_by_id), 'cert_label_counts': _label_counts(final_ids, rows_by_id), 'excluded_from_test_count': len(guide_ids) + len(final_ids) + len(train_ids), 'uses_full_dataset_as_test': len(pool_test_ids) == len(data_ids), 'guide_ids_used_for_eval': False, 'cert_ids_used_as_final_split': True}
...
def main() -> None:
    ...
    summary_path = output_artifact_path(args.summary_path, output_path.with_suffix('.summary.json'))
    ...
    if outputs_exist and args.cache_policy == 'reuse':
        print(summary_path.read_text(encoding='utf-8').strip())
        return
    summary = build_pcss_eval_split(data_path=data_path, split_ids_path=split_ids_path, train_rows_path=train_rows_path, output_path=output_path, summary_path=summary_path)
