...
from mias_dcms.data import PairExample, filter_examples_by_ids, load_examples
from mias_dcms.utils import read_json, read_jsonl as read_jsonl_file, resolve_input_path, resolve_output_path, write_json
...
class StageCacheDecision:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'stage_name': self.stage_name, 'cache_policy': self.cache_policy, 'cache_hit': bool(self.cache_hit), 'action': self.action, 'existing_outputs': list(self.existing_outputs), 'missing_outputs': list(self.missing_outputs)}
...
def stage_cache_decision(*, stage_name: str, required_outputs: Iterable[str | Path], cache_policy: str) -> StageCacheDecision:
    ...
    if existing and missing:
        raise RuntimeError(f'{stage_name} partial cache: existing outputs={existing}; missing outputs={missing}. Use --cache_policy overwrite to regenerate this stage, or remove the partial outputs.')
...
def summarize_teacher_label_usage(rows: Iterable[dict[str, Any]], *, purpose: str) -> dict[str, Any]:
    ...
    groundtruth_calls = sum((1 for row in materialized if row.get('teacher_source') == 'groundtruth_substitute_for_real_teacher_api'))
    unknown_calls = sum((1 for row in materialized if row.get('teacher_source') not in {'teacher_api_file', 'groundtruth_substitute_for_real_teacher_api'}))
    return {'purpose': purpose, 'teacher_calls': int(api_calls + groundtruth_calls + unknown_calls), 'teacher_api_file_calls': int(api_calls), 'groundtruth_substitute_calls': int(groundtruth_calls), 'unknown_teacher_source_calls': int(unknown_calls), 'estimated_prompt_tokens': estimate_query_document_prompt_tokens(materialized), 'estimated_completion_tokens': int(len(materialized))}

def embedding_usage_payload(*, embedding_source: str | Path, row_count: int, embedding_dim: int, purpose: str) -> dict[str, Any]:
    return {'purpose': purpose, 'embedding_source': str(embedding_source), 'embedding_rows': int(row_count), 'embedding_dim': int(embedding_dim), 'embedding_value_count': int(row_count) * int(embedding_dim)}
...
def load_stage_examples(*, data_path: str | Path, query_field: str, document_field: str, label_field: str) -> list[PairExample]:
    return load_examples(resolve_input_path(data_path, PROJECT_ROOT), query_field=str(query_field), document_field=str(document_field), label_field=str(label_field))

def split_examples(examples: list[PairExample], split_payload: dict[str, Any]) -> tuple[list[PairExample], list[PairExample]]:
    ...
    return (filter_examples_by_ids(examples, guide_ids), filter_examples_by_ids(examples, pool_ids))
...
def train_label_snapshot(rows: Iterable[dict[str, Any]]) -> dict[str, int]:
    return {str(row['id']): binary_to_int(row.get('label', row.get('groundtruth')), field_name='train label snapshot') for row in rows}
...
def _teacher_logit_margin(row: dict[str, Any]) -> float | None:
    value = _first_present(row, ('teacher_logit_margin', 'teacher_logit', 'logit_margin', 'answer_logit_margin'))
...
def _normalize_teacher_row(row: dict[str, Any], *, source: str, teacher_temperature: float) -> dict[str, Any]:
    ...
    label_value = _first_present(row, ('teacher_label', 'teacher_answer', 'parsed_answer', 'answer', 'label', 'groundtruth'))
    ...
    confidence_value = _first_present(row, ('teacher_confidence', 'confidence', 'parsed_confidence', 'answer_confidence'))
    ...
    payload: dict[str, Any] = {'id': sample_id, 'teacher_label': int(label), 'teacher_confidence': float(confidence), 'teacher_source': 'teacher_api_file', 'teacher_label_source': label_source, 'teacher_confidence_source': confidence_source}
...
def load_teacher_labels(path: str | Path, *, teacher_temperature: float=1.0) -> dict[str, dict[str, Any]]:
    ...
    for index, row in enumerate(_iter_teacher_json_rows(source), start=1):
        normalized = _normalize_teacher_row(dict(row), source=f'{source}:{index}', teacher_temperature=teacher_temperature)
...
def runtime_args_from_cli(cli_args: argparse.Namespace) -> argparse.Namespace:
    ...
    return argparse.Namespace(max_length=int(value('max_length', 512)), batch_size=int(value('batch_size', 4)), eval_batch_size=int(value('eval_batch_size', 32)), epochs=int(value('epochs', 3)), lr=float(value('lr', 0.0002)), weight_decay=float(value('weight_decay', 0.01)), gradient_accumulation_steps=int(value('gradient_accumulation_steps', 4)), max_grad_norm=float(value('max_grad_norm', 1.0)), warmup_ratio=float(value('warmup_ratio', 0.1)), early_stopping_patience=value('early_stopping_patience', None), early_stopping_min_delta=float(value('early_stopping_min_delta', 0.0)), threshold=float(value('threshold', 0.0)), num_workers=int(value('num_workers', 2)), prefetch_factor=int(value('prefetch_factor', 2)), pad_to_multiple_of=int(value('pad_to_multiple_of', 8)), cache_tokenization=bool(value('cache_tokenization', True)), pin_memory=bool(value('pin_memory', True)), tf32=bool(value('tf32', True)), torch_dtype=str(value('torch_dtype', 'auto')), trust_remote_code=bool(value('trust_remote_code', True)), lora_r=int(value('lora_r', 1)), lora_alpha=int(value('lora_alpha', 16)), lora_dropout=float(value('lora_dropout', 0.05)), lora_target_modules=str(value('lora_target_modules', 'attention_mlp')), lora_layer_scope=str(value('lora_layer_scope', 'all')), balance_train_classes=bool(value('balance_train_classes', False)), seed=int(value('seed', 42)))
