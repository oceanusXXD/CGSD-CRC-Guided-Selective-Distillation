...
from mias_dcms.embedding_text import chunk_text_for_embedding, format_pair_embedding_text, mean_pool_vectors, normalize_vectors
from mias_dcms.utils import resolve_input_path, resolve_output_path, write_json
...
def load_transformers_embedding_model(model_path: str | Path, *, device_name: str, dtype_name: str) -> tuple[Any, torch.nn.Module, torch.device, bool, torch.dtype]:
    ...
    model_kwargs = {'trust_remote_code': True, 'local_files_only': True, 'torch_dtype': dtype}
    try:
...
def load_vllm_embedding_model(model_path: str | Path, *, dtype_name: str, max_length: int, tensor_parallel_size: int, gpu_memory_utilization: float, enforce_eager: bool) -> Any:
    from vllm import LLM
    return LLM(model=str(model_path), runner='pooling', convert='embed', trust_remote_code=True, dtype=resolve_vllm_dtype(dtype_name), tensor_parallel_size=int(tensor_parallel_size), gpu_memory_utilization=float(gpu_memory_utilization), enforce_eager=bool(enforce_eager), max_model_len=int(max_length))

def embed_texts_transformers(*, model: torch.nn.Module, tokenizer: Any, device: torch.device, texts: list[str], batch_size: int, max_length: int, uses_lm_hidden_states: bool) -> np.ndarray:
    vectors: list[np.ndarray] = []
    safe_batch_size = max(1, int(batch_size))
    for start in range(0, len(texts), safe_batch_size):
        batch_texts = texts[start:start + safe_batch_size]
        encoded = tokenizer(batch_texts, padding=True, truncation=True, max_length=int(max_length), return_tensors='pt')
        encoded = {key: value.to(device) for key, value in encoded.items()}
        with torch.inference_mode():
            outputs = model(**encoded, output_hidden_states=uses_lm_hidden_states, use_cache=False, return_dict=True)
...
def embed_texts_vllm(*, model: Any, texts: list[str], batch_size: int) -> np.ndarray:
    vectors: list[np.ndarray] = []
    safe_batch_size = max(1, int(batch_size))
    for start in range(0, len(texts), safe_batch_size):
        batch_texts = texts[start:start + safe_batch_size]
...
def pair_texts_for_row(row: dict[str, Any], *, query_field: str, document_field: str, mode: str, target_chars: int, overlap_chars: int) -> list[str]:
    ...
    chunks = chunk_text_for_embedding(document, target_chars=int(target_chars), overlap_chars=int(overlap_chars))
...
def build_embeddings(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if backend == 'transformers':
        tokenizer, model, device, uses_lm_hidden_states, dtype = load_transformers_embedding_model(args.model_path, device_name=str(args.device), dtype_name=str(args.torch_dtype))
        dtype_label = str(dtype).replace('torch.', '')
        device_label = str(device)
    elif backend == 'vllm':
        model = load_vllm_embedding_model(args.model_path, dtype_name=str(args.torch_dtype), max_length=int(args.max_length), tensor_parallel_size=int(args.tensor_parallel_size), gpu_memory_utilization=float(args.gpu_memory_utilization), enforce_eager=bool(args.enforce_eager))
        device_label = 'vllm'
    else:
    ...
    def flush_pending() -> None:
        ...
        if backend == 'transformers':
            ...
            flat_vectors = embed_texts_transformers(model=model, tokenizer=tokenizer, device=device, texts=batch_texts, batch_size=int(args.request_batch_size), max_length=int(args.max_length), uses_lm_hidden_states=uses_lm_hidden_states)
        else:
            flat_vectors = embed_texts_vllm(model=model, texts=batch_texts, batch_size=int(args.request_batch_size))
        ...
        if dimension is None:
            ...
            memmap = np.lib.format.open_memmap(output_path, mode='w+', dtype=np.float32, shape=(total_rows, dimension))
    ...
    for row in rows[row_cursor:]:
        ...
        formatted = pair_texts_for_row(row, query_field=str(args.query_field), document_field=str(args.document_field), mode=str(args.mode), target_chars=int(args.target_chars), overlap_chars=int(args.overlap_chars))
