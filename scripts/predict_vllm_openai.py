...
from scripts.cli_common import add_stage_cache_args, estimate_query_document_prompt_tokens, input_artifact_path, load_selected_train_rows, load_split_ids, load_stage_examples, load_teacher_labels, output_artifact_path, output_dir_from_arg, print_existing_stage_result, read_jsonl, selected_train_rows_path, split_examples, split_ids_path, stage_cache_decision, summarize_teacher_label_usage, train_label_snapshot, write_stage_usage, StageCacheDecision
from mias_dcms.binary_protocol import BINARY_SCORE_SOURCE, normalize_binary_token
from mias_dcms.data import PairExample, format_binary_chat_prompt
...
def _validate_lora_checkpoint(*, checkpoint_dir: Path, model_path: Path) -> tuple[Path, int]:
    ...
    missing = [str(path) for path in (model_config_path, adapter_dir, adapter_config_path) if not path.exists()]
    ...
    if missing:
        raise FileNotFoundError(f'LoRA checkpoint is incomplete; vLLM would fail to load it. Missing: {missing}. Expected a trained round checkpoint at {checkpoint_dir}.')
    ...
    if recorded_model_path and (not _same_existing_path(str(recorded_model_path), model_path)):
        raise ValueError(f'LoRA checkpoint base model does not match the vLLM base model. checkpoint model_path={recorded_model_path!r}, requested model_path={str(model_path)!r}.')
    adapter_config = read_json(adapter_config_path)
    if str(adapter_config.get('peft_type', '')).upper() != 'LORA':
        raise ValueError(f"Adapter {adapter_config_path} is not a PEFT LoRA adapter: peft_type={adapter_config.get('peft_type')!r}")
...
def _start_vllm_server(args: argparse.Namespace, *, checkpoint_dir: Path | None) -> subprocess.Popen[str]:
    ...
    cmd = [str(args.python_executable or sys.executable), '-m', 'vllm.entrypoints.openai.api_server', '--model', str(model_path), '--host', host, '--port', str(port), '--served-model-name', served_model_name, '--disable-uvicorn-access-log', '--uvicorn-log-level', 'warning', '--gpu-memory-utilization', f'{max(0.05, min(float(args.gpu_memory_utilization), 0.98)):.3f}', '--max-num-seqs', str(max(1, int(args.max_num_seqs)))]
    ...
    if checkpoint_dir is not None:
        adapter_dir, adapter_rank = _validate_lora_checkpoint(checkpoint_dir=checkpoint_dir, model_path=model_path)
        max_lora_rank = max(int(args.max_lora_rank or 0), adapter_rank, 16)
        cmd.extend(['--enable-lora', '--max-lora-rank', str(max_lora_rank), '--lora-modules', f'{lora_model_name}={adapter_dir}'])
    log_path = output_artifact_path(args.server_log_path, PROJECT_ROOT / 'experiments' / 'runs' / 'vllm_openai.log') if args.server_log_path else PROJECT_ROOT / 'experiments' / 'runs' / 'vllm_openai.log'
    ...
    try:
        proc = subprocess.Popen(cmd, cwd=str(PROJECT_ROOT), env=env, stdin=subprocess.DEVNULL, stdout=handle, stderr=subprocess.STDOUT, close_fds=True, text=True, start_new_session=True)
...
def _client(args: argparse.Namespace) -> Any:
    ...
    return OpenAI(api_key=str(args.api_key or 'EMPTY'), base_url=str(args.base_url), timeout=float(args.timeout), max_retries=0, http_client=httpx.Client(timeout=httpx.Timeout(float(args.timeout)), limits=httpx.Limits(max_connections=max_connections, max_keepalive_connections=max_connections)))
...
def _predict_one(*, client: Any, model_name: str, example: PairExample, args: argparse.Namespace) -> dict[str, Any]:
    last_exc: Exception | None = None
    attempts = max(1, int(args.request_retries))
    for attempt in range(attempts):
        try:
            prompt = _completion_prompt(example)
            response = client.completions.create(model=model_name, prompt=prompt, temperature=float(args.temperature), max_tokens=int(args.max_tokens), logprobs=int(args.top_logprobs))
            ...
            if one_lp is None and zero_lp is None:
                raise RuntimeError(f'missing 1/0 logprobs for output={content!r}; for this model family this usually means the raw prompt did not end after the empty no-thinking block before scoring.')
            if one_lp is None:
                one_lp = -100.0
            if zero_lp is None:
                zero_lp = -100.0
            ...
            row.update({'id': str(example.sample_id), 'query': example.query, 'document': example.document, 'label': int(example.label), 'groundtruth': int(example.label), 'score': score, 'zero_logit': float(zero_lp), 'one_logit': float(one_lp), 'probability': probability, 'prediction': int(prediction), 'generated_text': str(int(prediction)), 'score_source': SCORE_SOURCE, 'round_index': int(args.round_index), 'vllm_raw_text': content})
            return row
        except Exception as exc:
            last_exc = exc
            if attempt + 1 >= attempts:
                print(json.dumps({'event': 'prediction_request_failed', 'sample_id': str(example.sample_id), 'attempts': attempts, 'error': repr(exc)}, ensure_ascii=False, sort_keys=True), file=sys.stderr, flush=True)
                break
...
def _predict_many(*, client: Any, model_name: str, examples: list[PairExample], args: argparse.Namespace, output_path: Path, partial_path: Path) -> list[dict[str, Any]]:
    ...
    print(json.dumps({'prediction_cache_rows': len(existing_by_id), 'pending_rows': len(pending), 'total': len(examples), 'parallel_requests': workers}, sort_keys=True), flush=True)
    if pending:
                ...
                while in_flight:
                    ...
                    if not done:
                        if progress_timeout > 0 and now - last_progress_at >= progress_timeout:
                            stale = sorted(({'sample_id': sample_id, 'in_flight_seconds': round(now - started_at, 1)} for sample_id, started_at in in_flight.values()), key=lambda item: item['in_flight_seconds'], reverse=True)[:20]
                            message = f'no vLLM predictions completed before progress timeout; completed={completed}, total={len(examples)}, in_flight={len(in_flight)}, progress_timeout={progress_timeout:.1f}s'
                            print(json.dumps({'event': 'prediction_progress_timeout', 'completed': completed, 'total': len(examples), 'in_flight': len(in_flight), 'progress_timeout_seconds': progress_timeout, 'stale_samples': stale}, ensure_ascii=False, sort_keys=True), file=sys.stderr, flush=True)
                    ...
                    for future in done:
                        ...
                        try:
                            ...
                            print(json.dumps({'event': 'prediction_future_failed', 'sample_id': sample_id, 'error': error}, ensure_ascii=False, sort_keys=True), file=sys.stderr, flush=True)
                        else:
        ...
        if failures:
            ...
            raise RuntimeError(f'vLLM prediction failed after retries; failures={len(failures)}, completed={completed}, total={len(examples)}, preview={preview}')
...
def main() -> None:
    ...
    guide_predictions_path = output_artifact_path(args.guide_predictions_path, round_dir / 'guide_student_predictions.jsonl')
    final_predictions_path = output_artifact_path(args.final_predictions_path, round_dir / 'final_student_predictions.jsonl')
    pool_predictions_path = output_artifact_path(args.pool_predictions_path, round_dir / 'pool_student_predictions.jsonl')
    partial_predictions_path = output_artifact_path(args.partial_predictions_path, round_dir / 'all_student_predictions.partial.jsonl')
    train_label_snapshot_path = output_artifact_path(args.train_label_snapshot_path, round_dir / 'predict_train_label_snapshot.json')
    ...
    if args.show_result:
        ...
        return
    required_outputs = [all_predictions_path, guide_predictions_path, final_predictions_path, pool_predictions_path, train_label_snapshot_path, usage_path]
    try:
        cache_decision = stage_cache_decision(stage_name='predict_vllm_openai', required_outputs=required_outputs, cache_policy=args.cache_policy)
        ...
        if args.cache_policy == 'reuse' and partial_predictions_path.exists():
            ...
            cache_decision = StageCacheDecision(stage_name='predict_vllm_openai', cache_policy='reuse', cache_hit=False, action='run', existing_outputs=existing, missing_outputs=missing)
            print(json.dumps({'stage_name': 'predict_vllm_openai', 'cache_policy': 'reuse', 'partial_cache_resume': True, 'existing_outputs': existing, 'missing_outputs': missing}, ensure_ascii=False, sort_keys=True), flush=True)
        else:
            raise
    ...
    try:
        ...
        examples = load_stage_examples(data_path=args.data_path, query_field=args.query_field, document_field=args.document_field, label_field=args.label_field)
        ...
        teacher_labels_by_id = load_teacher_labels(args.teacher_labels_path, teacher_temperature=float(args.teacher_temperature)) if args.teacher_labels_path else {}
        selected_train_rows = read_jsonl(input_artifact_path(args.selected_train_rows_path, selected_train_rows_path(output_dir))) if args.selected_train_rows_path else load_selected_train_rows(output_dir)
        write_json(train_label_snapshot(selected_train_rows), train_label_snapshot_path)
        predictions = _predict_many(client=client, model_name=request_model_name, examples=prediction_examples, args=args, output_path=all_predictions_path, partial_path=partial_predictions_path)
        ...
        write_stage_usage(usage_path, {'stage_name': 'predict_vllm_openai', 'round_index': int(args.round_index), 'cache': cache_decision.to_dict(), 'student_model_calls': len(predictions), 'student_model_role': 'base_model' if checkpoint_dir is None else 'round_lora_adapter', 'vllm_base_url': str(args.base_url), 'vllm_model': request_model_name, 'parallel_requests': int(args.parallel_requests), 'partial_predictions_path': str(partial_predictions_path), 'estimated_student_prompt_tokens': estimate_query_document_prompt_tokens(predictions), 'estimated_student_completion_tokens': len(predictions), 'teacher_label_usage': teacher_usage, 'groundtruth_substitute_calls': teacher_usage['groundtruth_substitute_calls'], 'teacher_api_file_calls': teacher_usage['teacher_api_file_calls'], 'all_predictions_path': str(all_predictions_path), 'guide_predictions_path': str(guide_predictions_path), 'final_predictions_path': str(final_predictions_path), 'pool_predictions_path': str(pool_predictions_path)})
