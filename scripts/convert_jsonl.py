...
from mias_dcms.utils import resolve_input_path, resolve_output_path, write_jsonl
...
def convert_rows(rows: list[dict[str, Any]], *, id_field: str='id', query_field: str='query', document_field: str='document', label_field: str='groundtruth', fixed_query: str | None=None, id_prefix: str='row') -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        label = normalize_binary_label(_row_value(row, label_field, source=source), field_name=f'{source} {label_field}')
        payload = dict(row)
        payload.update({'id': sample_id, 'query': str(query), 'document': str(document), 'groundtruth': int(label)})
...
def main() -> None:
    ...
    converted = convert_rows(rows, id_field=str(args.id_field), query_field=str(args.query_field), document_field=str(args.document_field), label_field=str(args.label_field), fixed_query=args.fixed_query, id_prefix=str(args.id_prefix))
